...
export class PubSubPublisher {
  ...
  async publish(data: Buffer): Promise<string> {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const error = new Error(
            `Failed to publish to PubSub: ${response.status} ${response.statusText} - ${errorText}`,
          );
          ...
          if (
            ...
            logger.warn(
              `PubSub publish attempt ${attempt} failed, retrying in ${delayMs}ms:`,
              {
                error: error.message,
        ...
        if (attempt > 1) {
          logger.info(`PubSub publish succeeded on attempt ${attempt}:`, {
            messageId,
          });
        } else {
    ...
    throw (
      lastError || new Error("Unknown error occurred during PubSub publish")
    );
  }
}
